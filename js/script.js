document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatColorToFilename(colorValue) {
        ...
        // Convertit en minuscules, remplace les espaces (et autres blancs) par des underscores
        return colorValue.toLowerCase().trim().replace(/\s+/g, '_');
    }
    ...
    function setupVariantSelection(containerSelector, itemSelector, selectedDisplaySelector, attributeName) {
        ...
        items.forEach(item => {
            item.addEventListener('click', () => {
                ...
                // Si l'élément cliqué est une pastille de couleur (swatch)
                if (item.classList.contains('swatch')) {
                    ...
                    if (splatIframe && selectedValue) {
                        // Formate le nom de la couleur pour l'utiliser comme nom de dossier
                        const folderNamePart = formatColorToFilename(selectedValue);

                        // --- MODIFICATION ICI ---
                        // Construit la nouvelle source : /nom_couleur_formaté/index.html
                        const newSplatSrc = `/${folderNamePart}/index.html`;
                        // --- FIN MODIFICATION ---

                        // Vérifie si la source a réellement changé avant de recharger
                        if (splatIframe.getAttribute('src') !== newSplatSrc) {
                            console.log("Changement du splat vers:", newSplatSrc);
                            splatIframe.classList.remove('loaded'); // Cache l'iframe pendant le chargement
                            if (splatLoadingIndicator) {
                                splatLoadingIndicator.textContent = "Chargement Vue 3D...";
                                splatLoadingIndicator.classList.add('visible'); // Affiche l'indicateur
                            }
                            splatIframe.setAttribute('src', newSplatSrc); // Change la source de l'iframe
                        }
                    }
                }
            });
    ...
    // Applique la logique aux sélecteurs de couleur (finition)
    ...
    footerToggles.forEach(toggle => {
        toggle.addEventListener('click', () => {
            ...
            if (listToToggle) {
                ...
                // Le CSS gère le changement visuel basé sur l'attribut aria-expanded
            }
        });
    });
